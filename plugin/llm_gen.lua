...
local config = {
  ...
  init = function(options)
    local ollama_status, _ = pcall(io.popen, "docker start ollama > /dev/null 2>&1 &")
    if not ollama_status then
      pcall(io.popen, "docker start ollama")
      pcall(io.popen, "docker start ollama > /dev/null 2>&1 &")
    end
  end,
...
map({ "n", "v" }, "<leader>gg", ":Gen Generate_Code<CR>", { desc = "[G]enerate code according to spec" })
