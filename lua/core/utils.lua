...
function M.set_new_hl()
  ...
  for _, hlgroup in ipairs(to_italic) do
    update_hl(hlgroup, {
      italic = true,
      force = true,
      fg = vim.api.nvim_get_hl(0, { name = hlgroup }).fg
    })
  end
  for _, hlgroup in ipairs(to_bold) do
    update_hl(hlgroup, {
      bold = true,
      force = true,
      fg = vim.api.nvim_get_hl(0, { name = hlgroup }).fg
  ...
  for _, hlgroup in ipairs(to_bold_italic) do
    update_hl(hlgroup, {
      italic = true,
      bold = true,
      force = true,
...
function M.gh(plugin)
  return 'https://github.com/' .. plugin
end

function M.cb(plugin)
